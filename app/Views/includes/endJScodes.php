...
function profilePhoto(imgUrl,div) {
	var coveruploadfield = $('coveruploadfield').val();
	if (imgUrl.files && imgUrl.files[0]) {
		var reader = new FileReader();
		reader.onload = function (e) {
      $('#profilePhotoPreview_'+div).attr('src', e.target.result);
					$('#profilePhotodivPreview_'+div).css({"background":"url('"+e.target.result+"') no-repeat center center","background-size":"cover"});
		}
...
function accept_deliver(id){
	$.ajax({
		type:'POST',
		url:'<?php echo base_url(); ?>Includes/accept_deliver',
		data:{'id':id},
		beforeSend: function(){
...
function deliverphoto(imgUrl) {
	if (imgUrl.files && imgUrl.files[0]) {
		var reader = new FileReader();
		reader.onload = function (e) {
			$('#photo_deliver').css({"background":"url('"+e.target.result+"') no-repeat center center","background-size":"cover"});
		}
		reader.readAsDataURL(imgUrl.files[0]);
	}
}
function itemphoto(imgUrl) {
	if (imgUrl.files && imgUrl.files[0]) {
		$('#photo_item').html("");
		for(var i = 0; i < imgUrl.files.length; i++){
			var reader = new FileReader();
			reader.onload = function (e) {
				$('#photo_item').append("<img src='"+e.target.result+"' class='col-md-3' alt='' />");
			}
